      ...
      def self.emit(ir)
        ...
        if components["parameters"]
          components["parameters"].each do |p_name, p|
            if p["schema"] && p["schema"]["$ref"]
              type = p["schema"]["$ref"].split('/').last
            else
              type = p.dig("schema", "type") || "string"
            end
            opts = []
        ...
        if components["headers"]
          components["headers"].each do |h_name, h|
            if h["schema"] && h["schema"]["$ref"]
              type = h["schema"]["$ref"].split('/').last
            else
              type = h.dig("schema", "type") || "string"
            end
            opts = []
        ...
        if components["responses"]
          components["responses"].each do |r_name, resp|
            if resp["content"]
              ...
              schema_name = ref ? ref.split('/').last : "Object"
              opts = []
              opts << "required:#{resp['content'][media_type]['required']}" unless resp['content'][media_type]['required'].nil?
              opts_str = opts.empty? ? "" : " #{opts.join(' ')}"
        ...
        if components["links"]
          components["links"].each do |l_name, l|
            ...
            if l["requestBody"]
              l_opts << "requestBody:#{l['requestBody']}"
            end
        ...
        all_ops.each do |op_entry|
          ...
          path_data.each do |method, op|
            next if method == "summary" || method == "description"
            ...
            if op["responses"]
              op["responses"].each do |status, resp|
                ...
                next if status == "200" && resp["description"] == "OK" && !resp["content"] && !resp["headers"] && !resp["links"] && !resp["$ref"]
                ...
                if resp["headers"]
                  resp["headers"].each do |h_name, h|
                    if h["$ref"]
                      ...
                      if h["schema"] && h["schema"]["$ref"]
                        h_type = h["schema"]["$ref"].split('/').last
                      else
                        h_type = h.dig("schema", "type") || "string"
                      end
                      h_opts = []
            ...
            if op_type == "webhook"
              out += "# @webhook #{method.upcase} #{path}\n"
            else
              out += "# @route #{method.upcase} #{path}\n"
            end
