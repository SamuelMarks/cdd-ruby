        ...
        tokens.each_with_index do |token, i|
          if token[1] == :on_kw && token[2] == 'class'
            ...
            if token[2] =~ /#\s*@api_client\s+([A-Z]+)\s+(\S+)/
              current_http_method = ::Regexp.last_match(1).downcase
              current_http_path = ::Regexp.last_match(2)
            ...
            while j < tokens.size && tokens[j][1] != :on_nl && tokens[j][1] != :on_semicolon && !(tokens[j][1] == :on_kw && tokens[j][2] == 'end')
              if tokens[j][1] == :on_label
                ...
                if current_http_path && current_http_method && ir.openapi_spec.dig('paths', current_http_path,
                                                                                   current_http_method)
