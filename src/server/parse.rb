      ...
      def self.parse(tokens, ir)
        ...
        tokens.each_with_index do |token, i|
          if token[1] == :on_ident && %w[get post put delete patch options head trace query].include?(token[2])
            ...
            next_string = tokens[i+2] || tokens[i+3] # simplified
            if next_token && next_token[1] == :on_sp
              current_method = token[2]
            end
            ...
            openapi_path.scan(/\{(\w+)\}/).flatten.each do |param_name|
              ...
              unless ir.openapi_spec["paths"][openapi_path][current_method]["parameters"].any? { |p| p["name"] == param_name && p["in"] == "path" }
                ir.openapi_spec["paths"][openapi_path][current_method]["parameters"] << {
                  "name" => param_name,
                  "in" => "path",
                  "required" => true,
