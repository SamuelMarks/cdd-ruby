...
from_keys_cli += ["openapi", "info", "paths", "title", "operationId", "components", "servers", "tags", "externalDocs", "security"]
from_keys_server += ["openapi", "paths", "components", "servers", "info", "tags", "security"]
...
md_files.each do |file, sets|
  ...
  new_lines = lines.map do |line|
    if line =~ /^\| \*\*(.*?)\*\* \| `\[([ x])\]` , `\[([ x])\]` \|/
      ...
      if obj_col =~ /\(`([^`]+)`\)/
        field = $1
      elsif obj_col =~ /([^ ]+) Object/
        field = $1.downcase
      elsif obj_col =~ /OpenAPI Object \(Root\)/
        field = "openapi"
      end
      ...
      is_to = true if ["info", "openapi", "servers", "tags", "webhooks", "externalDocs", "components", "paths", "securitySchemes", "schemas", "parameters", "requestBodies", "headers", "links", "callbacks", "responses", "discriminator", "xml", "propertyName", "mapping", "defaultMapping"].include?(field)
      
      is_from = true if file == 'servers.md' && ["paths", "openapi", "method"].include?(field)
      is_from = true if file == 'client-sdk-cli.md' && ["paths", "openapi", "operationId", "info", "title", "method"].include?(field)
