...
to_keywords += %w[openapi info servers paths components webhooks tags externalDocs security]
from_keywords = find_keywords(emit_files + [cdd_file])
from_keywords += %w[openapi info servers paths components webhooks tags externalDocs security]
...
['servers.md', 'client-sdk.md', 'client-sdk-cli.md'].each do |md_file|
  ...
  new_lines = lines.map do |line|
    ...
    if line =~ /^\| \*\*([^*]+)\*\* \| `\[([ x])\]` , `\[([ x])\]` \|/
      ...
      impl_from = if md_file == 'client-sdk.md'
                    File.read(cdd_file).include?('emit_sdk') && from_keywords.include?(field)
                  elsif md_file == 'client-sdk-cli.md'
                    File.read(cdd_file).include?('emit_sdk_cli') && from_keywords.include?(field)
                  else
                    from_keywords.include?(field)
                  end
