...
class WebhooksTest < Minitest::Test
  def test_webhook
    ...
    assert_equal "Return a 200 status to indicate that the data was received successfully", op["responses"]["200"]["description"]
    
    out = Cdd::Routes::Emitter.emit(ir)
    assert_match(/@request_body \[Pet\] application\/json Information about a new pet/, out)
