...
class ServerVariablesTest < Minitest::Test
  def test_server_variables
    ...
    assert_equal %w[v1 v2], srv['variables']['version']['enum']
    ...
    assert_equal %w[us-east eu-west], osrv['variables']['region']['enum']

    out = Cdd::Routes::Emitter.emit(ir)
    assert_match(%r{@server_var https://api.example.com/\{version\} version v1 enum:v1,v2 description:API version},
                 out)
    assert_match(%r{@op_server_var https://\{region\}\.api\.example\.com region us-east enum:us-east,eu-west}, out)
  end
