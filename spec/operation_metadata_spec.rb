...
class OperationMetadataTest < Minitest::Test
  def test_operation_metadata
    ...
    assert_match(/# @external_docs https:\/\/example.com\/docs User docs/, emitted)
    assert_match(/# @op_server https:\/\/us.example.com\/api US server/, emitted)
    assert_match(/# @param id \[integer\] in:path required:true style:simple explode:false allowReserved:true allowEmptyValue:false deprecated:true The user ID/, emitted)
    assert_match(/# @request_body \[User\] application\/json required:true A new user object/, emitted)
  end
end
