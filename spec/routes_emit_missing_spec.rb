...
class RoutesEmitMissingTest < Minitest::Test
  def test_routes_emit_missing_lines
    ...
    ir.openapi_spec["security"] = [{"oauth2" => ["read", "write"]}]
    
    op = {
      "parameters" => [
        {"name" => "id", "in" => "query", "required" => true, "schema" => {"type" => "integer"}, "description" => "ID param"}
      ],
      "requestBody" => {
    ...
    assert_match(/# @server http:\/\/test Test server/, out)
    assert_match(/# @security oauth2 read, write/, out)
    assert_match(/# @param id \[integer\] in:query required:true ID param/, out)
    assert_match(/# @request_body \[MyInput\] application\/json Body desc/, out)
    assert_match(/# @response 201 \[MyOutput\] application\/json Created/, out)
