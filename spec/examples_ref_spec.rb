...
class ExamplesRefTest < Minitest::Test
  def test_examples_ref
    ...
    assert_equal '#/components/examples/ValidBodyExample',
                 op['requestBody']['content']['application/json']['examples']['ValidBody']['$ref']
    assert_equal '#/components/examples/ValidRespExample',
                 op['responses']['200']['content']['application/json']['examples']['ValidResp']['$ref']

    out = Cdd::Routes::Emitter.emit(ir)
    assert_match(/@param_example_ref q ValidQuery ValidExample/, out)
    assert_match(%r{@request_body_example_ref application/json ValidBody ValidBodyExample}, out)
    assert_match(%r{@response_example_ref 200 application/json ValidResp ValidRespExample}, out)
  end
end
